...
# C:/Users/jinwook/Dropbox (Visual Cognition Lab)/문서/Visual Cognition Lab/Exp_JinWook/Experiment/Multisensory Vection/Experiment 2/Experiment Data/Eye
...
#participantList = ['2022_10_17 14_40_20_P01', '2022_10_17 19_09_08_P02', '2022_10_20 14_28_23_P04','2022_10_20 19_30_40_P05','2022_10_21 14_39_07_P06', '2022_10_24 19_38_40_P09']
...
def getBlinkList(partiNum, blockNum):
    # Read "gaze.pldata" and "gaze_timestamps.npy" data
    fullPath = path + partiNum + "/" + blockNum + "/"
    get_data = load_pldata_file(fullPath, "blinks")

    data_blink = get_data.data
...
def countBlinkRate(blink, marker):
    for b in blink:
        #print(b)   # blink 시점
        for m in marker:
            #print(m)    # m[3] = start /  m[4] = endtime   (1s = 1)
            if len(m) != 6:
                continue

            #print(m)
            if m[3] + 2 <= b and b <= m[4]:
                m[5] += 1
                break
...
def saveResult2CSV(partNum, marker):
    print(partNum + ' Saving Result...')

    header = ['Audio','Optic','TrialNum','startTime','endTime','cnt']
    import csv
    with open('./Blink Data/' + partNum + '_Blink Rate.csv', 'w', newline='') as f:
        writer = csv.writer(f)
...
def main():
    for p in participantList:
        fullList = []
        for b in blockList:
            blinks = getBlinkList(p, b)
            marker = getEventMarkers(p, b)
            marker = countBlinkRate(blinks, marker)
            fullList.extend(marker)
